...
HRESULT CServiceManager::Create(_In_z_ LPCWSTR szServiceNameW, _In_ LPCREATEINFO lpCreateInfo)
{
                                                               ...
                                                               lpCreateInfo->nServiceType == eServiceType::NetworkService))
    {
        ...
        sReqPrivInfoW.pmszRequiredPrivileges =
            (LPWSTR)((lpCreateInfo->szRequiredPrivilegesW != NULL && lpCreateInfo->szRequiredPrivilegesW[0] != 0)
                         ? (lpCreateInfo->szRequiredPrivilegesW)
                         : L"");
    ...
    // setup restart time
    if (SUCCEEDED(hRes) && (lpCreateInfo->nServiceType == eServiceType::LocalSystem ||
                            lpCreateInfo->nServiceType == eServiceType::NetworkService))
    {
