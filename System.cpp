...
HRESULT GetOpSystemInfo(_Out_ CStringW &cStrOpSystemW)
{
    ...
    if (cStrOpSystemW.Concat(L" ") == FALSE || cStrOpSystemW.Concat(szTempW) == FALSE)
    {
        return E_OUTOFMEMORY;
    }
    // done
...
HRESULT GetAllUsers(_Inout_ TArrayListWithFree<LPWSTR> &aUsersList)
{
    ...
    for (i = 0; i < (DWORD)MX_ARRAYLEN(aBuiltinUsersW); i++)
    {
        ...
        if (SUCCEEDED(hRes))
        {
            ...
            if (SUCCEEDED(hRes))
            {
                if (cStrTempW.InsertN(L"/", 0, 1) == FALSE || cStrTempW.Insert(aBuiltinUsersW[i], 0) == FALSE ||
    ...
    do
    {
        ...
        if (nStatus == NERR_Success || nStatus == ERROR_MORE_DATA)
        {
            for (i = 0; i < dwEntries; i++)
            {
                ...
                if (SUCCEEDED(hRes2))
                {
                    if (cStrTempW.ConcatN(L"/", 1) == FALSE || cStrTempW.Concat(lpUserInfo0[i].usri0_name) == FALSE ||
                        cStrTempW.InsertN((cSid.IsAnyWellKnownSid() != FALSE) ? L"+" : L"-", 0, 1) == FALSE)
                    {
                        hRes2 = E_OUTOFMEMORY;
                    }
    ...
    do
    {
        ...
        if (nStatus == NERR_Success || nStatus == ERROR_MORE_DATA)
        {
            for (i = 0; i < dwEntries; i++)
            {
                ...
                if (SUCCEEDED(hRes2))
                {
                    if (cStrTempW.ConcatN(L"/", 1) == FALSE || cStrTempW.Concat(lpGroupInfo0[i].lgrpi0_name) == FALSE ||
                        cStrTempW.InsertN((cSid.IsAnyWellKnownSid() != FALSE) ? L"+" : L"-", 0, 1) == FALSE)
                    {
                        hRes2 = E_OUTOFMEMORY;
                    }
