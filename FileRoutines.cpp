...
HRESULT RemoveDirectoryRecursive(_In_ LPCWSTR szFolderNameW, _In_opt_ FileRoutines::eDelayedDelete nDD)
{
    ...
    dwAttributes = ::GetFileAttributesW(szFolderNameW);
    ...
    {
        return MX_HRESULT_FROM_WIN32(ERROR_DIRECTORY);
    }
    if ((dwAttributes & FILE_ATTRIBUTE_REPARSE_POINT) != 0)
    {
        if (nDD == FileRoutines::eDelayedDelete::WaitUntilReboot)
        {
            if (::MoveFileExW(szFolderNameW, NULL, MOVEFILE_DELAY_UNTIL_REBOOT) == FALSE)
            {
                return MX_HRESULT_FROM_LASTERROR();
            }
        }
        else if (::RemoveDirectoryW(szFolderNameW) == FALSE)
        {
            hRes = MX_HRESULT_FROM_LASTERROR();
            if (nDD != FileRoutines::eDelayedDelete::DeleteOnRebootOnFailure ||
                ::MoveFileExW(szFolderNameW, NULL, MOVEFILE_DELAY_UNTIL_REBOOT) == FALSE)
            {
                return hRes;
            }
        }
...
HRESULT ConvertToLongPath(_Inout_ CStringW &cStrPathW)
{
    ...
    else if ((sW[0] >= L'A' && sW[0] <= L'Z') || (sW[0] >= L'a' && sW[0] <= L'z') && sW[1] == L':')
    {
...
HRESULT ConvertToNative(_Inout_ CStringW &cStrPathW)
{
    ...
    // try DOS volume/path
    if (((sW[0] >= L'A' && sW[0] <= L'Z') || (sW[0] >= L'a' && sW[0] <= L'z')) && sW[1] == L':')
    {
    ...
    *lphFile = NULL;
    if (StrNCompareW(szFileNameW, L"\\??\\", 4) == 0 || StrNCompareW(szFileNameW, L"\\Device\\", 8, TRUE) == 0)
    {
        ...
        for (dwRetry = CREATE_RETRIES_COUNT; dwRetry > 0; dwRetry--)
        {
            ...
            if (hRes != HRESULT_FROM_WIN32(ERROR_SHARING_VIOLATION) &&
                hRes != HRESULT_FROM_WIN32(ERROR_LOCK_VIOLATION) && hRes != E_ACCESSDENIED)
            {
                break;
            }
        }
                              ...
                              _In_opt_ BOOL bReplaceExisting, _In_opt_ DWORD dwSharedMode)
{
    ...
    if (StrNCompareW(szFileNameW, L"\\??\\", 4) == 0 || StrNCompareW(szFileNameW, L"\\Device\\", 8, TRUE) == 0)
    {
        ...
        for (dwRetry = CREATE_RETRIES_COUNT; dwRetry > 0; dwRetry--)
        {
            ...
            ::MxMemSet(&sIoStatus, 0, sizeof(sIoStatus));
            nNtStatus = ::MxNtCreateFile(lphFile, FILE_GENERIC_READ | FILE_GENERIC_WRITE, &sObjAttrib, &sIoStatus, NULL,
                                         FILE_ATTRIBUTE_NORMAL, (ULONG)dwSharedMode,
